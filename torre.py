...
class Torre(peca.Peca):
    ...
    def validar(self, posicao_x, posicao_y):
        if super().validar(posicao_x, posicao_y):
            if (posicao_x != self._posicao_atual_x) and (posicao_y != self._posicao_atual_y):
                return False

            if ((posicao_x == self._posicao_atual_x or posicao_y == self._posicao_atual_y) and
                    (posicao_x != self._posicao_atual_x or posicao_y != self._posicao_atual_y)):
                return True
